...
cd /opt && sudo git clone https://github.com/gibiansky/IHaskell &&  sudo chown -R $DATASCI_USER:$DATASCI_USER $IHaskell 

cd $IHaskell && stack install gtk2hs-buildtools --install-ghc && \
stack --install-ghc install ihaskell && ihaskell install --prefix=$CONDA_DIR