...
# USER $DATASCI_USER
ENV R_BASE_VERSION=3.4.1
RUN conda install --yes -c conda-forge icu readline libtool ncurses
RUN conda remove --yes libedit
RUN conda install --yes -c conda-forge readline
...
USER root
RUN apt-get update && apt-get install --yes --no-install-recommends libedit2 libgl1-mesa-dri libgl1-mesa-glx
RUN apt-get update && apt-get install --yes vim nano
USER $DATASCI_USER
