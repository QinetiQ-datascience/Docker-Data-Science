...
# Dev Tools
apt update && apt --yes install software-properties-common python-software-properties build-essential automake
apt update && apt --yes install  wget curl unzip zip bzip2 fonts-liberation python-dev libjpeg8-dev zlib1g-dev gfortran  fonts-dejavu ca-certificates \
...
ghostscript gnuplot graphviz less xvfb libxss1 libgconf-2-4 libnss3 libasound2 net-tools
...
# Install Nodejs
apt update && apt install --yes nodejs-legacy npm

# Version control
apt update && apt install --yes git subversion mercurial --no-install-recommends
...
# Install Boost
apt update && add-apt-repository universe && apt update && apt --yes install  libboost-all-dev